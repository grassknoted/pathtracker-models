...
def _cfg(url='', **kwargs):
    return {
        ...
        'crop_pct': 1., 'interpolation': 'bicubic',
...
default_cfgs = {
    ...
    #     mean=(0.008, 0.002, 0.008), std=(0.9, 0.4, 0.9),
    # ),
    'vit_base_patch16_224': _cfg(
        url='https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-vitjx/jx_vit_base_p16_224-80ecf9dd.pth',
...
class TimeSformerIN(nn.Module):
    def __init__(self, img_size=32, patch_size=16, num_classes=400, num_frames=8, attention_type='divided_space_time',  pretrained_model='', **kwargs):
        super(TimeSformerIN, self).__init__()
        self.pretrained=True
        # Original
...
class TimeSformerINSpace(nn.Module):
    def __init__(self, img_size=32, patch_size=16, num_classes=400, num_frames=8, attention_type='divided_space_time',  pretrained_model='', **kwargs):
        super(TimeSformerIN, self).__init__()
        self.pretrained=True
        # Original
...
class TimeSformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, num_classes=400, num_frames=8, attention_type='divided_space_time',  pretrained_model='', **kwargs):
        ...
        # Skinnier
        self.model = VisionTransformer(img_size=img_size, num_classes=num_classes, patch_size=patch_size, embed_dim=64, depth=4, num_heads=4, mlp_ratio=0.5, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, num_frames=num_frames, attention_type=attention_type, **kwargs)
